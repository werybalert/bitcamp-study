...
public class MemberHandler {
  ...
  static MemberHandler[] members = new MemberHandler[MAX_SIZE];
  static int userId = 1;
  static int length = 0;
  ...
  public static void printMembers(Number[] MemberHandler) {
    ...
    for (int i = 0; i < length; i++) {
      Number m = MemberHandler[i];
      System.out.printf("%s, %s, %s\n", m.name, toGenderString(m.gender));
    }
  } // printMembers 끗

  public static void viewMember(Number[] MemberHandler) {
    String memberNo = Prompt.inputString("번호? ");
    for (int i = 0; i < length; i++) {
      Member m = MemberHandler[i];
